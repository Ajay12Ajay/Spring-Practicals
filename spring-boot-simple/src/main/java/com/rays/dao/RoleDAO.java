...
public class RoleDAO {
	...
	public List<RoleDTO> search(RoleDTO dto, int pageNo, int pageSize) {
		...
		// CriteriaBuilder :Query bnane ke liye use hta h
		...
		/*
		 * // SELECT * FROM RoleDTO // create final select query cq.select(qRoot);
		 */
		...
		if (dto != null) {
			...
			if (dto.getDiscriptions() != null && dto.getDiscriptions().length() > 0) {
				predicateList.add(builder.like(qRoot.get("discriptions"), dto.getDiscriptions() + "%"));
			}
		}
